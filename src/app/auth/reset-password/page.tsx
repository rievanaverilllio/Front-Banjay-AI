...
function ResetPasswordContent() {
	...
	return (
		<div className="min-h-screen w-full flex relative bg-[#FAFAF5]">
			{/* Back Home */}
			<Link href="/landing_page" className="absolute top-6 left-6 z-20 flex items-center px-4 py-2 border border-gray-300 text-gray-700 rounded-full bg-white hover:bg-gray-100 hover:text-black transition font-medium text-sm shadow">
			...
			>
				...
				{status !== "success" && (
					<motion.p variants={fadeInFromBottom} className="text-sm text-gray-600 mb-6">Buat password baru untuk akun kamu. <Link href="/auth/login" className="text-black hover:underline">Kembali login?</Link></motion.p>
				)}
				...
				{status === "success" ? (
					<motion.div variants={fadeInFromBottom} className="max-w-md bg-green-50 border border-green-200 text-green-700 rounded-lg p-4">
						<p className="font-semibold mb-1">Password berhasil direset!</p>
						<p className="text-sm">Mengalihkan ke halaman login...</p>
						<div className="mt-4 flex gap-2">
							<Link href="/auth/login" className="flex-1 text-center py-2 rounded-full bg-black hover:bg-gray-800 text-white font-semibold text-sm transition">Pergi ke Login sekarang</Link>
						</div>
					</motion.div>
				) : (
...
export default function ResetPasswordPage() {
	return (
		<Suspense fallback={<div className="min-h-screen flex items-center justify-center text-sm text-gray-600">Memuat...</div>}>
